...
const SeriesDetailPage = () => {
    ...
    return (
        <div className="flex flex-col gap-5 text-white pb-10">
            ...
            <span className="md:text-[30px] text-[25px] mx-auto font-semibold">{movie?.name}</span>
            <div className="flex flex-row gap-5 justify-center">
                <button
                    onClick={async () => {
                        ...
                        if (isMarked) {
                            newArray = newArray.filter((item) => JSON.stringify(item) !== JSON.stringify(seriesItem));
