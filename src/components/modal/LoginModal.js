    ...
    return (
        <div className="max-w-[400px] p-5 bg-white rounded-2xl mx-5 flex justify-center gap-5 flex-col items-center">
            ...
            <form className="flex flex-col gap-5 justify-center items-center" onSubmit={handleSubmit(onSubmit)}>
                ...
                >
                    {isSubmitting ? (
                        <div className="w-4 h-4 border-r-transparent block rounded-full border-[4px] border-white animate-spin"></div>
                    ) : (
                        'Log In'
                    )}
                </button>
                <span>
                    Not a member?
                    <Link to={'/signup'} className="text-secondary ml-2 hover:underline" onClick={() => setShow(false)}>
                        Sign Up
                    </Link>
                </span>
            </form>
