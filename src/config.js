...
export const tmdbSeries = {
    ...
    getSeriesDetails: (id, detail) =>
        `${endpointSeries}/${id}${detail ? `/${detail}` : ''}?api_key=${apiKey}&language=en-US`,
    getSeriesSearchPage: (query, page = 1) => `${seriesSearchEndpoint}&query=${query}&page=${page}`,
    getSeriesGenreList: (genre, page = 1) =>
        `http://api.themoviedb.org/3/discover/tv?api_key=${apiKey}&with_genres=${genre}&page=${page}`,
};
